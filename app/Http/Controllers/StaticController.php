...
class StaticController extends Controller
{
    ...
    {
        $data = ['source' => 'Я робот переводчик!',"lang" => 'ru-en'];

        $translate = curl_init();
        curl_setopt($translate, CURLOPT_URL, "https://fasttranslator.herokuapp.com/api/v1/text/to/text");
        curl_setopt($translate, CURLOPT_POST, true);
        curl_setopt($translate, CURLOPT_POSTFIELDS, http_build_query($data));
        curl_setopt($translate, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($translate, CURLOPT_RETURNTRANSFER, true);
        $response = json_decode(curl_exec($translate));
